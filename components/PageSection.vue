...
<template>
  ...
  >
    ...
    <UnLazyImage v-if="background" :src="`./images/${background}`" alt="" class="absolute h-full w-full object-cover -z-1" />
    <div v-if="overlay" class="absolute inset-0 overflow-x-hidden lg:overflow-visible">
      <UnLazyImage :src="`./images/${overlay}`" :class="`absolute ${overlayPosition}`" />
    </div>
