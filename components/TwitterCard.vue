...
<template>
  <div class="twitter-card mb-8 break-inside-avoid rounded-xl bg-brand-light p-4">
    <div class="mb-4 w-full flex items-center gap-2">
      <img :src="`~/assets/images/twitter-profile-${handle}.jpg`" :alt="handle" class="h-9 w-9 flex-none rounded-full">
      <div class="flex-1">
        <p class="font-bold">
          {{ name }}
        </p>
    ...
    <img v-if="image" :src="`~/assets/images/${image}`" class="mb-4 w-full">
    <div v-if="retweet" class="mb-4 border rounded-md p-3">
      <div class="mb-2 w-full flex items-center gap-1">
        <img :src="`~/assets/images/twitter-profile-${handle}.jpg`" :alt="handle" class="h-5 w-5 flex-none rounded-full">
        <p class="flex-1">
      ...
      <img v-if="retweet.image" :src="`~/assets/images/${retweet.image}`" class="mt-4 w-full">
    </div>
    <p>
      {{ time }} • {{ date }}
    </p>
