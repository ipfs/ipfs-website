...
const addHooks = async (instance) => {
  instance.nuxt.hook('generate:done', async () => {
    ...
    for (const htmlFile of htmlFiles) {
      ...
      // ----- new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))
      seds(
        /basePath:"\/ipfs\/hash\/"/gm,
        htmlFile,
        "basePath:(ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))), ipfsMatch?ipfsMatch[0]:'/')"
      );
      seds(
        /assetsPath:"\/_nuxt\/"/gm,
        htmlFile,
        "assetsPath:(ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))), ipfsMatch?ipfsMatch[0]:'.') + '/_nuxt/'"
      );
      seds(
        ...
        (_, start, end) => {
          // - diff: /\\/ipfs\\/[^/]+/
          return `staticAssetsBase:(ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))),(ipfsMatch?ipfsMatch[0]:'.'))+"\u002F_nuxt\u002Fstatic\u002F${end}`;
        }
      );
      seds(
        /<base href="\/ipfs\/hash\/">/,
    ...
    for (const jsFile of jsFiles) {
      seds(
        /"\/ipfs\/hash\/_nuxt\/\"/,
        jsFile,
        '(ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))), ipfsMatch?ipfsMatch[0]+\'\/_nuxt\/\':\'\/_nuxt\/\')'
      );
      seds(
        /base: '\/ipfs\/hash\/'/gm,
        jsFile,
        "base:(ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))), ipfsMatch?ipfsMatch[0]:'/')"
      );
      seds(
      ...
      seds(
        ...
        jsFile,
        "return __webpack_require__.p.replace(\'./\', (ipfsMatch=window.location.pathname.match(new RegExp(^(\/(?:ipfs|ipns)\/[^/]+))), ipfsMatch?ipfsMatch[0]+\'/\':\'/\'))"
      );
    }
  });
};
